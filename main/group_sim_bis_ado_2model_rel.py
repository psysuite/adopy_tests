...
Uses 2-model relative ADOpy approach with random PSE/JND values.
...
def simulate_subject(subj_id, pse, jnd, ntrials, output_dir, file_prefix, logger):
    ...
    # Initialize ADOpy wrappers (pre and post)
    offset = 500
    ...
    for trial_id in range(ntrials):
        # Select model
        use_pre = (trial_order[trial_id] == 'pre')
        exp = exp_pre if use_pre else exp_post
        
        # Get magnitude from ADOpy
        magnitude = exp.get(use_pre)
        
        # Convert to absolute stimulus
        if use_pre:
        ...
        row = {
            ...
            'model': 'pre' if use_pre else 'post',
            'user_ans': user_ans
        }
        rows.append(row)
    ...
    # Create combined experiment for statistics
    exp_combined = qw.BISRelADOpyWrapper(
        adoparams={"guess_rate": 0.5, "lapse_rate": 0.04, "noise_perc": 0.1},
        taskparams={"min": 0, "max": 300, "offset": offset, "ntrials": ntrials}
    )
    exp_combined.stimuli_ms = exp_pre.stimuli_ms + exp_post.stimuli_ms
    exp_combined.responses = exp_pre.responses + exp_post.responses
    exp_combined.successes = exp_pre.successes + exp_post.successes
    ...
    # Analyze results
    result_dict = safe_analyze_subject(exp_combined, subject_row, output_dir, file_prefix, logger)
    
    # Get experiment statistics
    exp_stats = exp_combined.print_statistics()
...
def main():
    ...
    # Set up logger
    log_file = os.path.join(output_dir, "sim_2model_rel.log")
    logger = setup_logger("sim_2model_rel", log_file=log_file)
    ...
    if all_rows_list:
        ...
        plot_group_model_histograms(all_rows_list, output_dir, file_prefix, offset)
