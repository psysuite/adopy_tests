...
Uses REL1 (1-model relative) ADOpy approach.
...
GBF filename format: SXX_GZ_PSE_JND_REL1.txt
  - XX: subject number within group (zero-padded)
  - Z:  group index
  - PSE/JND: jittered values rounded to int (used for simulation)

Output:
  - GBF files: data/output/sim_gridrnd/REL1/
  - Plots:     data/output/sim_gridrnd/REL1/group_PSE_JND/results/
  - Excel:     data/output/sim_gridrnd/REL1/group_PSE_JND/results/
  - Grid/metric plots: data/output/sim_gridrnd/REL1/
...
BIS_PARAMS              = {"min": 1, "max": 300, "offset": OFFSET, "ntrials": N_TRIALS}
    ...
    for group_idx, (pse_center, jnd_center) in enumerate(grid, 1):
        ...
        if not SKIP_PHASES_1_2:
            ...
            for subj_in_group in range(1, N_SUBJECTS_PER_GROUP + 1):
                subj_counter += 1
                # Jitter and round to int — this value is used for simulation AND filename
